...
function checkStatus(d1, d2, d3 ,d4){
    (d1 == 0 && d2 == 0 && d3 == 0 && d4 == 0) ? all_not_on = true  : all_not_on = false
...
cron.schedule("* * * * *",async()=>{
    ...
    avg = (device1+device2+device3+device4)/(device1_status+device2_status+device3_status+device4_status+all_not_on)
